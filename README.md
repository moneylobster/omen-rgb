...
td.show("TRAINING RUN 42", color=(255, 128, 0), scroll=True)    # scroll, any length
td.show(98.42, color=(0, 200, 255), prefix="ACC:", suffix="%")  # number, auto-scrolls
```
...
For training progress where you want a readable two-digit percent, pass
`vertical=True` to `show()`. The library includes a native 4-wide × 6-tall
vertical font; two digits fill the 12-LED strip exactly, each spanning all 4
sticks:
...
for epoch in RGBtqdm(range(epochs), desc="epoch"):
    loss = train_epoch()
    td.show(loss, prefix="L", decimals=2,
...
- **Adjacent RAM LEDs bleed.** Individual LEDs are ~3mm apart and the
  diffusers overlap. Use `spacing=2`, vertical mode (only 2 wide glyphs),
